...
export function createDrizzleTransaction(): RunInTransaction {
  ...
  return async (fn) =>
    // runTx opens a per-call neon-serverless transaction (#493): the neon-http
    // driver getDb() uses can't run interactive transactions on CF Workers.
    runTx(async (tx) => {
