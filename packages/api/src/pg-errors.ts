...
/**
 * reviews unique seal (#1067, retightened #1158): one review per booking per
 * subject. A 23505 on this name means the same side re-submitted the same subject —
 * the submission service edits the existing hidden row instead of inserting a second
 * (a renter still reviews OPERATOR and VEHICLE separately — different subjects).
 * Keyed on (bookingId, subject) — subject=RENTER is the operator side — so two staff
 * of one operator can't both review one booking. Matching by name keeps this apart
 * from any future reviews unique.
 */
