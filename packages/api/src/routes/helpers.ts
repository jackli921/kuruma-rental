...
export function parseDateRange(
  ...
  // Exactly one provided (both-absent is handled above). A single local guard
  // both returns the 400 and narrows fromParam/toParam to `string`, so the Date
  // constructions need no non-null assertions (#732).
  if (!fromParam || !toParam) {
    return {
      ok: false,
