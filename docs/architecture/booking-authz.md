...
| `POST /:id/cancel` | service | renter (own) + operator + bypass-admin operator-bound | Tiered cancellation (72h / 48h / 24h / same-day) is a renter-facing feature, so there is **no route gate**; the service authorizes renter-own and operator-tenant. #1260: because there is no gate, a bypass admin could otherwise cancel ANY tenant's booking by raw id (a refund-moving cross-tenant write), so it must bind to the operator it picked via `?operatorId=` — same 422/404 contract as `/status`. Renters (renter scope) and operators (tenant scope) are already clamped by `findById`, so they need no `operatorId`. |
