...
Revision 5 (2026-07-02): final implementation-readiness pass — verdict needs-minor-edits, then resolved. B1 — removed an internal contradiction (thread the effective `operatorId` uniformly; no "operators omit the param"). B2 — invite Button + `inviteOpen` state move into `OperatorTeamData` too (else P2b regresses for invite). B3 — build the badge label locally from `OperatorSummary[]` (`operatorNameById` is empty when a pick is active). B4 — make c1 additive: `OperatorNotFoundError extends NotFoundError` and KEEP the `admin.ts:47` local catch (pins `provider-invites.test.ts:152`). B5 — state the child prop type + delete dead `requireOwnOperator`. Now ready to plan.
...
- Delete the now-dead private `requireOwnOperator` (`operator-team.ts:142-145`) — all five callers are replaced, so lint flags it otherwise.
...
The no-existence-oracle seal above governs *member/invite* ids (a foreign one is indistinguishable from a missing one). A bogus *operator* id is a different matter: reads return an empty 200; `inviteStaff` calls `inviteService.createInvite`, which throws `OperatorNotFoundError` (`provider-invite.ts:20` class, `:74` throw) for an unknown operator.

**Bug to fix in slice 2 (c1):** `OperatorNotFoundError extends Error`, NOT `NotFoundError`, and the global handler (`error-handlers.ts`) has no branch for it — the only 404 mapping is a *local* catch in `routes/admin.ts:47`. Today the team path never reaches it (an operator's own id always exists), but this design makes it reachable for the first time (a picker-admin can supply an arbitrary `?operatorId=`), so it would currently 500. **Fix additively: make `OperatorNotFoundError extends NotFoundError`** so the existing `NotFoundError -> 404` branch catches it. **Keep the `routes/admin.ts:47` local catch as-is** — do NOT "DRY it away": it pins the suffix-free `'Operator not found'` message that `provider-invites.test.ts:152` asserts (the error's own `.message` carries an `: <operatorId>` suffix). This makes the fix zero-behavior-change for the existing admin caller. Add a route/integration test asserting a bogus `?operatorId=` on team invite is 404, not 500.
...
- **Thread the effective id uniformly (B1 — resolve the contradiction).** The child only renders with a resolved id, so pass the **effective** `operatorId` (an operator's own id, or a picker-admin's picked id) — NOT "undefined for operators." `fetchTeamMembers(operatorId)` / `fetchTeamInvites(operatorId)` always append `?operatorId=<operatorId>`. The server ignores it for operator roles (`resolveTeamOperatorId` returns `ctx.operatorId`) and honors it only for a picker-admin, so threading an operator's own id is harmless — it trades #904's "client names no operatorId" wire ethos for one uniform shape. There is **no** "operator omits the param" assertion; update `operator-team-api.test.ts` to the uniform shape.
- Query keys fold in the id: `teamMembersQueryOptions(operatorId)` / `teamInvitesQueryOptions(operatorId)` gain a **required** `operatorId` arg (they take none today) and key on `[...TEAM_MEMBERS_QUERY_KEY, operatorId]` so switching tenants refetches instead of serving a stale roster.
- **Invalidation rule (c4):** keep the exported `TEAM_INVITES_QUERY_KEY` / `TEAM_MEMBERS_QUERY_KEY` as the **2-element prefix**; the dialogs keep invalidating that prefix. React Query prefix-matches, so a prefix invalidation still hits every folded `[...prefix, operatorId]` key (safe over-invalidation). Do NOT convert the dialogs to invalidate a specific `[...prefix, id]`.
...
- **Render gate — a scoped, keyed child `OperatorTeamData` (P1 + P2b).** The current route calls `useSuspenseQuery(teamMembersQueryOptions())` + invites at the component *top level* (`team.tsx:55-56`). A loader prefetch guard does NOT stop that — `useSuspenseQuery` fetches on render if uncached — so in all-mode it would still issue the illegal `/operators/me/*` reads. The two team queries (and the invite/revoke/deactivate dialog open+selected state, `team.tsx:57-59`) must move into a child rendered **only when `operatorId` exists**:
  - `OperatorTeamRoute` (parent) keeps the always-legal `useSuspenseQuery(sessionQueryOptions())`, resolves the capability-gated `operatorId`, computes `canManage = canWriteAsOperatorOwner(session, pickedOperatorId)`, and renders either the all-mode prompt or `<OperatorTeamData operatorId={operatorId} canManage={canManage} csrfToken={session.csrfToken} key={operatorId} />`. Prop type: `{ operatorId: string; canManage: boolean; csrfToken: string }`. The parent header keeps only the title + subtitle + badge.
  - `OperatorTeamData` (new child) holds `useSuspenseQuery(teamMembersQueryOptions(operatorId))` + invites, `TeamView`, the three dialogs, **and all their local state — including the invite Button (`team.tsx:76-81`) and its `inviteOpen` state**, plus `selectedInvite`/`selectedMember`. Moving `inviteOpen` into the child is load-bearing for P2b: the parent does NOT remount on a tenant switch, so a half-open invite dialog left in the parent would survive the switch and submit under the newly-picked operator — the exact bug P2b closes for revoke/deactivate.
  - The **`key={operatorId}`** (mirroring `settings.tsx:76`) remounts the child on a tenant switch — which both resets stale dialog/form intent (P2b) and re-fires the scoped reads. This is the render gate that makes "all-mode fires no team read" true, not the loader guard.
