...
| **Slice 4b (#405, landed 2026-06-04 — `9e33403`)** | `fee_schedules` table + CRUD. Slice 6 **reads** that table to build the snapshot; it does not create or modify it. |
...
| Precondition | Why | Status 2026-06-04 |
|---|---|---|
| **Slice 4a (#404 — insurance options) merged to `marketplace-pivot`** | Slice 6 reads active `insurance_options` so the renter can pick one during booking and the booking can snapshot the selected option's name/price/deductible. | **Merged** (`c439883`). Per-operator `insurance_options` table + CRUD live on pivot; `insurance_options_operatorId_id_unique` composite key present (`schema.ts`). |
| **Slice 4b (#405 — fee schedules) merged to `marketplace-pivot`** | Slice 6 reads `fee_schedules` rows to build `bookings.fee_snapshot`. Without the table + its `feeType`/`unit`/`amountJpy`/`vehicleClassId` columns there is nothing to snapshot. | **Merged** (`9e33403`). `fee_schedules` table + two partial-unique active-row indexes (`idx_..._operator_class` operator-wide vs class-scoped) present (`schema.ts`). |
| **Slice 4c (#406 — drop legacy class-level pricing) merged** | Pricing is now **vehicle-level only**: `vehicle_classes.dailyRateJpy/hourlyRateJpy` + their CHECKs are dropped (migration `0034`); `calculateBookingPrice(vehicle: VehicleRates, …)` prices off the assigned vehicle. Slice 6's `totalPrice` + price snapshot build on this, not on legacy class rates. | **Merged** (`605d089`). NOTE Neon staging may show migration drift on `0034` — see follow-up #428. |
| **Slice 5 (#391 — storefront search + vehicle selection) merged** | Slice 6 is the **submit** for the vehicle the renter selected in slice 5's storefront-detail screen. Slice 5 owns "renter picks a concrete vehicle"; slice 6 owns "renter confirms that vehicle into a booking." Proposal §6 critical path: "5 and 6 can partially parallelize" — but the submit endpoint needs slice 5's vehicle-selection UI as its caller. | **#391 still OPEN** — not yet landed. Hard precondition for the web submit + confirmation wiring. |
| **Slice 2 (#387 — locations) merged** | `bookings.pickup_location_id` + `dropoff_location_id` FKs (proposal §2 "Locations") and the substitution rule "same-operator, **same-location**" (§9 item 25) both require the `locations` table. Substitution's location check is unimplementable without it. | **Merged** (`22a889b`, PR #414). `locations` table + `defaultTurnaroundMinutes` (non-negative CHECK) + composite `locations_operatorId_id_unique` present (`schema.ts`). |
| **Slice 3 (#388 — ACRISS taxonomy + operator vehicle-class CRUD) merged** | Substitution's "same ACRISS class" rule (§5.5) keys off the `acriss_code` column on `vehicle_classes`. | **Merged** (`f4cd0bf`). `acriss_code` + operator-scoped vehicle-class CRUD on pivot. |
...
- Postgres only excludes rows where **every `=` operand is non-null**, so today's nullable `vehicleId` means unassigned (class-only) bookings never collide. That subtlety is mirrored in `InMemoryBookingRepository.create` (`packages/api/src/repositories/in-memory/booking.ts:125-149`, verified — the `data.vehicleId !== null` guard + per-row `existing.vehicleId === null` skip) and `getConflictingBookings` (`:12-23`, skips when `booking.vehicleId !== vehicleId`).
...
2. **Rebind the trigger** to compute `effectiveEndAt` from the **turnaround** value, not the legacy 60-min `bufferMinutes`. See §5.3 — `effective_end_at = end_at + turnaround_minutes` where turnaround resolves `locations.defaultTurnaroundMinutes ?? 2880` (48h) — location-only in MVP, no per-vehicle override. Proposal §2 "Vehicle turnaround buffer" + §9 item 20 ("rename/map `bufferMinutes` → `turnaround_minutes`; do not leave the old 60-minute default").
...
- `BookingRepository` interface (`packages/api/src/repositories/types.ts:240`) — `create(ctx, data)` is a single insert today.
- `RunInTransaction` (`types.ts:356`) + `createDrizzleTransaction` (`repositories/drizzle/transaction.ts`) **already exist** — used by `MaintenanceService`. Its current repo bundle is only `{ vehicleRepo, maintenanceLogRepo }` (verified `types.ts:356-361`). Slice 6 reuses this exact pattern to make booking-submit one transaction; it does not invent a new transaction mechanism. The factory's repo bundle is widened to include the booking + event repos (§4.2).
...
On `origin/marketplace-pivot` the `bookings` table is still the **legacy single-tenant shape** (verified `schema.ts` `bookings` table): `renterId`, `classId` (single FK → `vehicleClasses.id`, NOT a composite `(operatorId, classId)` seal yet), nullable `vehicleId` (single FK → `vehicles.id`), `startAt`/`endAt`/`effectiveEndAt`, `status`, `source`, `externalId`, `notes`, `idempotencyKey`, and the pricing/cancellation columns `totalPrice` (nullable integer), `cancellationFee`, `cancelledAt`. It has **no** `operatorId`, **no** `requestedVehicleId`/`assignedVehicleId`, **no** `pickupLocationId`/`dropoffLocationId`, **no** `bookingCode`, **no** `insuranceOptionId`/`insuranceSnapshot`, **no** `feeSnapshot`, **no** `currentState`. The only index is `idx_bookings_classId`. Slices 2-5 did not touch it (slices 2-4 added the surrounding tenant tables only). **Slice 6 is the booking-table marketplace migration.** (Proposal §5.1 step 4: "replace legacy nullable `bookings.vehicleId` with selected/assigned vehicle semantics.")

> **`totalPrice` already exists and is nullable (#406, slice 4c).** Post-4c, `calculateBookingPrice(vehicle, …)` prices off the assigned **vehicle**; a class-only (`vehicleId` null) booking persists `totalPrice: null`, and cancellation applies `booking.totalPrice ?? 0` (a null-total booking cancels for ¥0 — intentional + test-pinned today). **Slice 6 owns the fix (follow-up #429):** when this slice assigns a concrete vehicle (submit always selects one; substitution swaps one), it must compute + persist `totalPrice` from the assigned vehicle's rates (the price snapshot), and revisit the cancellation fee so the `?? 0` does not become the permanent answer for previously null-total rows. Add the #429 guard test: "assigning a vehicle to a null-total booking backfills `totalPrice`."
...
```
BEGIN
  1. Resolve class + requested vehicle (must belong to class; vehicle's operator owns it);
     derive assignedVehicleId := requestedVehicleId  (never client-supplied; operator
     may substitute later via §5.5)
  2. Resolve turnaround_minutes := location.defaultTurnaroundMinutes
                                   ?? 2880 (48h)         [§5.3]
        (pickup-location-level; no per-vehicle override in MVP)
  3. Compute server-side price from the ASSIGNED VEHICLE's rates
        (calculateBookingPrice(vehicle, startAt, endAt), #74/#406 — vehicle-level
         post-4c, never class-level, never client-supplied; non-null on submit) [§6]
...
```
turnaround_minutes = location.defaultTurnaroundMinutes   -- pickup location; default 2880 on the column
                     ?? 2880                              -- 48h hard fallback
```

- Proposal §2 "Vehicle turnaround buffer", §9 item 20, §10 item 11. `locations.default_turnaround_minutes` defaults to **2880 (48h)** per §5.1 step 4. **MVP is location-only** — the proposal's optional `vehicles.turnaround_minutes_override` (§9 item 20) is **deliberately not built**; if a vehicle ever needs a longer buffer, that override is a post-MVP follow-up.
- The existing trigger `compute_effective_end_at()` (`drizzle/0015`) reads `vehicles.bufferMinutes` (default 60). **Rebind it** to resolve `turnaround_minutes` from the location→default chain and key off `assignedVehicleId`'s pickup location. Keep the trigger (defense-in-depth) so a direct SQL insert can't bypass the buffer; the service also computes `effectiveEndAt` for the in-memory repo to mirror. CHECK `effectiveEndAt >= endAt` stays.
- **`bufferMinutes` retirement:** §9 item 20 is explicit — "do not leave the old 60-minute default in place." **Drop `vehicles.bufferMinutes` entirely** (no per-vehicle override in MVP) and add `locations.defaultTurnaroundMinutes` (default 2880). The exclusion range thus widens from "60 min after return" to "48h after return" — that is the intended behavior change.
...
## 6. Price, insurance + fee snapshot shapes

### 6.0 Vehicle-level price snapshot (`bookings.total_price`) — built on 4c, not legacy class pricing

Since slice 4c (#406, `605d089`) **dropped class-level rates** (`vehicle_classes.dailyRateJpy/hourlyRateJpy` + their CHECKs are gone), the only price source is the **vehicle**. `calculateBookingPrice(vehicle: VehicleRates, startAt, endAt)` (`@kuruma/shared/lib/pricing`) takes `{ dailyRateJpy, hourlyRateJpy }` off the **assigned vehicle**. Slice 6's submit always selects a concrete vehicle, so:

- `totalPrice` is computed server-side from the assigned vehicle's rates and is **non-null on every submit** (closing the post-4c gap where class-only bookings persisted `totalPrice: null`).
- This is the **price snapshot** referenced in follow-up **#429**: `totalPrice` is locked at booking time from the assigned vehicle's rates. Substitution (§5.5) re-snapshots `totalPrice` from the new assigned vehicle's rates in the same transaction, and must revisit the cancellation fee so the legacy `booking.totalPrice ?? 0` does not silently price a backfilled booking at ¥0. Guard test per #429: "assigning a vehicle to a (legacy) null-total booking backfills `totalPrice`."
...
| Layer | Cases |
|---|---|
| **Validator** (`shared/test/validators/booking`) | `requestedVehicleId` required UUID; **`assignedVehicleId` is NOT a client field** — server derives it (`= requestedVehicleId`) after scoped validation; optional `insuranceOptionId` UUID; reject end ≤ start (existing `.refine`); pickup/dropoff location ids required UUID; client-supplied `assignedVehicleId`/`totalPrice`/`bookingCode`/`operatorId`/snapshot fields silently dropped |
...
| **Turnaround unit** | `effectiveEndAt = endAt + resolve(locationDefault ?? 2880)`; location default wins over 2880 fallback; a location with `defaultTurnaroundMinutes = null` falls back to 2880; **mutation guard**: a 60-min result must FAIL (proves the legacy default is gone and the 48h default landed, §9 item 20) |
...
| **Service — substitution** | rejects different-operator vehicle (404); rejects different-location vehicle (400); rejects different-ACRISS-class vehicle (400); accepts same-operator/location/same-class → updates `assignedVehicleId`, leaves `requestedVehicleId`, appends `VEHICLE_SUBSTITUTED`; new vehicle already booked for range → `EXCLUSION_VIOLATION`→409, no event appended |
...
|---|---|---|---|
| Turnaround rebind leaves the legacy 60-min `bufferMinutes` default live (§9 item 20) | Medium | High | **Drop `vehicles.bufferMinutes`** (location-only MVP, no per-vehicle override); add `locations.defaultTurnaroundMinutes` default 2880; mutation-guard test asserts a 60-min result FAILS |
...
| ~~#387 (locations) not merged~~ — **RESOLVED 2026-06-04** (`22a889b`, PR #414): `locations` table present, FK + substitution location check now buildable | — | — | No longer a blocker |
| #391 (slice 5 storefront/vehicle-selection) not merged → no caller for the web submit + confirmation flow | High | Medium | #391 still OPEN (§1); land/coordinate slice 5 before the web sub-slice (§9 step 8), or stub the selection control and wire the real submit endpoint regardless (§11 item 2) |
| Neon staging migration drift on 4c's `0034` (#428) compounds with slice 6's two new migrations | Medium | Medium | Verify pivot Neon branch is at `0034` before generating slice 6 migrations; `db:verify` + CI `db-drift`; resolve #428 first if drift is present |
...
6. **Vehicle-level price snapshot / null-total backfill (#429).** Resolved: post-4c, pricing is vehicle-level only and class-only bookings carry `totalPrice: null`. Slice 6 owns follow-up #429 — every submit/substitution assigns a concrete vehicle and computes + persists `totalPrice` from that vehicle's rates (the price snapshot, §6.0), and revisits the cancellation `?? 0` so a backfilled booking is never silently priced at ¥0. This is in-scope for slice 6 (not a separate slice); #429 tracks the guard test.
