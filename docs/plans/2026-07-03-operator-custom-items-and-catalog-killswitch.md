...
Status: APPROVED - v2.4 (owner final review folded in 2026-07-03; decisions in §10). Tracking issue #1437.
Date: 2026-07-03
...
## 10. Decisions (owner final review, 2026-07-03 - RESOLVED)

- **A1 - RESOLVED (hide/freeze/reversible).** "off" = templates persist, picked items keep rendering, switch reversible. Physically deleting templates is rejected. (§1, §4.4 already encode this.)
- **D1 - RESOLVED (insurance IN SCOPE now, slice 3).** Owner wants independent insurance translations delivered in this effort, not deferred. It runs as its own multi-PR track AFTER slices 1-2 (it carries a wire-contract change and a from-scratch operator form). It does NOT block the kill-switch, since insurance has no catalog dependency today. Blast radius in §8 slice 3 / §8.3.
- **D3 - RESOLVED (fully hidden).** When the switch is OFF the `_admin/admin/templates` surface is fully hidden (not read-only-visible). `requirePlatformAdmin` authz is unchanged either way - the flag controls surfacing, never authorization (§4.4, §6.2).
- **D5 - RESOLVED (allow editing later).** An operator CAN edit a self-authored item's `nameI18n` after create (add ja/zh, fix en). The update path accepts `nameI18n` for self-authored rows, re-sealing `active_name_unique` on an `en` change (§7). Guard (both shapes): reject `nameI18n` on a PICKED row at the validator/service (friendly 4xx) so it never reaches the DB `CHECK` and 500s as a 23514.
- D2 (low stakes, deferred cleanup): the now-unused map-or-mint backfills (`backfillCatalogTemplates`/`backfillInsuranceTemplates`) and `auditInsuranceTemplates` - leave as inert history for now; delete when the insurance slice (3) lands. The `nullTemplateIdRowIds` audit half is deleted in slice 1 (§8.1).
- D4 (recorded, no action): `name` mirror stays an explicitly-written column, NOT a Postgres generated column. A generated `nameI18n->>'en'` would null out picked rows (their `nameI18n` is null), so it cannot be generated in this hybrid model.
