...
**Date:** 2026-05-25
**Status:** Proposal for review (Mr. Qiao / Jack)
...
| **Class display vs vehicle booking** | ACRISS class is the discovery/filtering/grouping layer; a concrete `vehicle_id` is selected before booking submit and is persisted on the booking. Postgres exclusion constraint on `(vehicle_id, time_range)` enforces uniqueness atomically. | Class-only booking would have a check-then-act race; vehicle-at-booking lets the DB constraint enforce uniqueness atomically |
| **Class taxonomy** | ACRISS 4-letter codes as canonical; i18n friendly labels in UI | Reversible (rename labels); adopting ACRISS aligns with OTA standard for future Trip.com sync |
...
4. Postgres exclusion constraint per `vehicle_id`
...
No data backfill (no data exists). Schema is **transformed** from baseline to marketplace shape: legacy class-level pricing removed, vehicleId required at booking, 48-hour default turnaround added, multi-tenancy + new tables added.
...
| `feature/<slug>` | Short-lived per-PR work for post-MVP slices | Created from `main`, deleted on merge |
...
| **3** | **ACRISS + vehicle CRUD** | `acriss_code` on vehicles, taxonomy seed, i18n labels, operator vehicle CRUD with plate + sha-ken expiry. Demo: operator adds a Toyota Yaris tagged "CCAR". | 2 |
...
| **6** | **Booking + event log** | Append-only `booking_events`, write-through to `bookings.current_state`, selected-vehicle booking, exclusion constraint, 48-hour default turnaround included in conflict range, `booking_code` generation, applicable `fee_schedules` snapshot to `bookings.fee_snapshot` at booking, confirmation page renders "potential additional charges". Demo: end-to-end booking with fee disclosure. | 3 |
...
| Exclusion constraint deadlocks under concurrent bookings | Low | Low | Already proven for single-tenant; adding `operator_id` doesn't change behavior (constraint is per-`vehicle_id`) |
...
4. **Start slice 1 (tenancy foundation)** — worktree on a feature branch off main; TDD vertical-slice per `~/.claude/rules/testing.md`; merge gate per §6.1.
