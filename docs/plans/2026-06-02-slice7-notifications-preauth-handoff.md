...
**Date:** 2026-06-02 (refreshed 2026-06-04)
**Status:** Draft v2 — refreshed 2026-06-04 against landed slices 3/4a/4b/4c (slice 6 still pending — hard prerequisite); awaiting green light
**Parent epic:** #385
...
| `bookings.booking_code`, `requested_vehicle_id`, `assigned_vehicle_id`, `insurance_option_id`, `insurance_snapshot`, `fee_snapshot`, `bookings.operatorId`, `booking_events` table | **Slice 6 (#392)**, proposal §5.1 step 4 + §9 item 3 | The notification payload reads these. **They are NOT on `marketplace-pivot` yet** (re-verified 2026-06-04: `bookings` still has the legacy `classId` + nullable-`vehicleId` shape — no `operatorId`/`booking_code`/`insurance_snapshot`/`fee_snapshot`; no `booking_events` table). See §1 preconditions. |
| `operators.pre_auth_handoff_url` **column** | **Already merged (slice 1, #386)** | Verified present on `marketplace-pivot` 2026-06-04 (`schema.ts:45`, `operators` table). Slice 7 *consumes* it; does not add it. |
...
| Precondition | Why | Status (re-verified 2026-06-04) |
|---|---|---|
| **Slice 6 (#392) merged to `marketplace-pivot`** | Slice 7's payload reads `booking_code`, `assigned_vehicle_id`, selected `insurance_snapshot`, `fee_snapshot`, `bookings.operatorId`, and fires off the post-commit hook slice 6 introduces. **Hard dependency** (proposal §6 "Depends on Slice 6"). | **Not started / not landed (2026-06-04).** Slices 3 / 4a / 4b / 4c landed since this plan was drafted, but #392 is not in `origin/marketplace-pivot`. `bookings` is still the legacy shape — no `bookingCode`/`operatorId`/`insuranceSnapshot`/`feeSnapshot`/event log. **This slice cannot start until #392 lands.** |
| `operators` table + `pre_auth_handoff_url` column | Confirmation page + email link out to it. | **Merged** (#386); `operators.pre_auth_handoff_url` present (`schema.ts:45`). **Correction (2026-06-04):** `OperatorRepository` already exists and slice 2 (#387) **already added `findById(id)`** (plus `findBySlug`/`create`/`existsBySlug`/`findSoleId`) — so slice 7 does **not** add `findById`. Note: the existing `findById(id)` is **unscoped** (no `CallerContext`); access is enforced in `OperatorService` (`services/operator.ts`), not the repo. Slice 7 reuses these; see §4e revised. |
...
| `index.ts:248-264` (on `origin/marketplace-pivot`, 2026-06-04) — provider chosen by env key; prod sentinel throws on use, dev stub returns a marker | `index.ts` — `EmailSender` chosen by `RESEND_API_KEY`; prod sentinel + dev console stub |
...
- **`NotificationDispatcher`** (`services/notification-dispatcher.ts`) is the post-commit hook. It is what the `TODO(#300)` in `BookingService` predicted: *"if a second post-booking side effect appears here, extract an outbox/event dispatcher rather than chaining another inline hook."* That condition is now true (thread + email). It exposes **two methods** so the durable enqueue and the network send are scheduled separately (see wiring above):
  - **`enqueue(booking)` — awaited, on the response path:**
    1. Builds the renter + operator notifications from the committed booking;
    2. For each: upsert a `QUEUED` `notification_log` row by `notify:<bookingId>:<kind>` (skip if already `SENT`). No network call — returns once the rows are durable.
  - **`flush(bookingId)` — deferred via `waitUntil`:**
    3. Loads the booking's non-`SENT` rows and `await emailSender.send(...)`; on success update `SENT` + `providerMessageId`; on throw, `console.error` (structured) + update `FAILED` + `error` + `attempts++`.
...
**Wiring the post-commit fire (no email inside the transaction — proposal §2 / §10 item 14):** split the dispatcher into an **awaited enqueue** and a **deferred send** so a `waitUntil` that never starts cannot lose the durable record. After the booking commits:

> 1. **`await dispatcher.enqueue(booking)`** — synchronously upserts the `QUEUED` `notification_log` rows (§4d steps 1–2). This is on the response path but cheap (one upsert per kind) and guarantees the durable record exists before the request returns.
> 2. **`c.executionCtx.waitUntil(dispatcher.flush(bookingId))`** — defers **only** the Resend send attempt (§4d step 3: `send` → `SENT`/`FAILED`). If the worker is evicted before `flush` starts, the `QUEUED` row survives, so the operator-portal resend (`POST /notifications/:id/resend`) or a later sweep retries it — nothing is lost.
>
> When slice 6's post-commit seam is present, `enqueue` runs inside it (still awaited); `flush` is always the deferred half. The response never waits for Resend, and the booking transaction never contains a network call.

### 4e. `OperatorRepository.findById` — already exists, reuse it (revised 2026-06-04)

**Correction:** the earlier draft assumed `OperatorRepository` had `create`/`existsBySlug` only and that slice 7 must add `findById`. Re-verified on `origin/marketplace-pivot`: the repository already exposes
```ts
// repositories/types.ts (existing — do NOT re-add)
create(data: { name; slug; preAuthHandoffUrl }): Promise<Operator>
existsBySlug(slug: string): Promise<boolean>
findSoleId(): Promise<string | null>
findById(id: string): Promise<Operator | undefined>   // slice 2, #387
findBySlug(slug: string): Promise<Operator | undefined>
```
So **slice 7 adds nothing here** — it consumes the existing `findById(id)` to read `pre_auth_handoff_url`. One caveat: this `findById(id)` is **unscoped** (no `CallerContext` arg); tenant access is enforced in `OperatorService` (`services/operator.ts`), not the repo. The post-commit dispatcher reads the operator by id directly (it runs on the platform's behalf, so unscoped is fine). If slice 6 instead surfaces the operator's pre-auth URL on the booking read model, prefer that and skip the operator-repo read entirely.

### 4f. Routes — `routes/notifications.ts`

- `GET /notifications?bookingId=` — operator-scoped list (management-read guard rejects `RENTER`, the only non-portal role; bypass callers need explicit `?operatorId=` or `?includeAll=true`, mirroring slice 4 §2). For the operator portal badge (proposal §4 business item 8).
...
Add, mirroring the `translationProvider` block (`index.ts:248-264` on `origin/marketplace-pivot`, 2026-06-04):
```ts
...
- Reads gated by a **management-read guard** (reject `RENTER`) *then* `operatorReadScope`: operator callers see only their `operatorId` rows; bypass callers (`PLATFORM_ADMIN` + legacy `STAFF`/`ADMIN`) see across, gated on `ctx.bypassScope === true` (not the role string).
...
| ~~**`OperatorRepository.findById`**~~ (removed 2026-06-04) | **Not slice 7's test.** `findById(id)` already exists and is covered by slice 2 (#387). It is unscoped by design (access enforced in `OperatorService`), so the earlier "operator resolves own row only / bypass resolves any" assertions don't apply at the repo layer. Slice 7 only relies on it returning `pre_auth_handoff_url` for a known id — covered indirectly by the dispatcher test. |
| **`NotificationService` + routes** | resend re-sends a `FAILED` row → `SENT`; `RENTER` read → 403; cross-operator resend id → 404; bypass list without `operatorId`/`includeAll` → 400. |
...
5. ~~`OperatorRepository.findById`~~ — **already exists (slice 2 #387); skip.** Just reuse it (or read pre-auth from the slice-6 booking read model).
...
**Modify (API):** `schema.ts` (add `notification_log` + enums), `repositories/types.ts` (add `NotificationLogRepository`; `OperatorRepository.findById` already exists — do NOT re-add), `index.ts` (DI: construct notification-log repo + email sender + dispatcher; reuse existing `operatorRepo`; post-commit seam + route mount + overrides), new migration in `drizzle/`. **Operator repos (`repositories/{drizzle,in-memory}/operator.ts`) already exist — reused, not modified.**
