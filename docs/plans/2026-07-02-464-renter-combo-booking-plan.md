...
## Pre-implementation notes (independent review 2026-07-02)

A third review pass verified all six load-bearing anchors above are still semantically true on current `develop`. Two watch-items to fold in during implementation — neither changes the plan's shape:

1. **Sold-out-at-submit is the one genuinely new failure mode — test it (Task 6/8).** The store-page `availableCount` is a point-in-time read; a class can sell out between render and submit. The server's advisory-lock guard returns `409 CLASS_COMBO_SOLD_OUT`, but no task currently asserts the wizard *surfaces* that. Add a Task 6 unit assertion (submit → 409 → renter sees a "no longer available" message, not a silent failure) and, optionally, a Task 8 e2e path. Specific-car booking already races, but its error code/message differ, so this path can't be assumed covered by existing tests.

2. **Confirm null `hourlyRateJpy` prices sub-day class-combo bookings as intended (Task 1/6).** Class rate plans carry only `dayRateJpy`; the plan sets `hourlyRateJpy: null`. The platform supports hourly granularity, and `estimateReservation`/`composeBookingTotal` already tolerate a null hourly rate — but confirm a short (e.g. 3-hour) class-combo booking prices to the intended value (day-rate floor), not an accidental zero/NaN. One assertion in the Task 6 pricing test pins this; no money-math redesign.
