...
| Rule | Why |
|------|-----|
| Routes import services and `routes/helpers.ts` only — never `repositories/`, not even `types.ts`. | Routes handle HTTP concerns; business logic belongs in services. Filter/entity shapes reach routes through the service layer (e.g. `services/filters.ts`), not the data layer. Two scoped carve-outs (#692): **sanctioned thin-read routes** (`regions`, `stats`) are read-only with no business policy — a pass-through service would be anemic — so they may DI *only* their `*Repository` interface from `types.ts` (an entity/filter import is query-shaping that belongs in a service, and stays blocked); and the lone **extraction-pending route** (`vehicles`) keeps a path-wide `types.ts` exemption transitionally until its `VehicleService` lands (#819). |
